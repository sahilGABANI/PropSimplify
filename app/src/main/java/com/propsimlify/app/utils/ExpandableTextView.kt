...
class ExpandableTextView @JvmOverloads constructor(
    ...
    fun toggle() {
        ...
        val amount = getColoredSpanned("Show less", ContextCompat.getColor(context, R.color.colorPrimary))
        val text1 = Html.fromHtml(amount)
        ...
        for (element in words) {
            if (element.isNotEmpty()) {
                if (element.contains("@")) {
                    ...
                    mention = "@$text"
                    mentionWithColor = getColoredSpanned(mention, ContextCompat.getColor(context, R.color.colorPrimary))
        ...
        text = if (collapsed) {
            if (lineCount == 3) expandDisplayedText
    ...
    private fun updateCollapsedDisplayedText(
        ...
        var mentionSpannableCollapsed: String = ""
            ...
            for (element in wordsCollapsed) {
                if (element.isNotEmpty()) {
                    if (element.contains("@")) {
                        ...
                        mentionWithColorCollapsed = getColoredSpanned(mentionCollapsed, ContextCompat.getColor(context, R.color.colorPrimary))
        ...
        collapsedDisplayedText = if (collapsedDisplayedText != null && collapsedDisplayedText!!.contains("Show More")){
            val readMore = getColoredSpanned("Show More", ContextCompat.getColor(context, R.color.colorPrimary))
            val read = mentionSpannableCollapsed.replace("Show More","")
            Html.fromHtml("$read $readMore")
        } else {
            Html.fromHtml(mentionSpannableCollapsed)
        }
//        val text1 = Html.fromHtml("$originalText... $amount")
//        expandDisplayedText = text1
        text = if (collapsed) collapsedDisplayedText else {
            collapsedDisplayedText
        }
    }
